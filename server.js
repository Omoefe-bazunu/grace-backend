...
// server.js (updated - added notice CRUD + read tracking)
require("dotenv").config();
const express = require("express");
const jwt = require("jsonwebtoken");
const bcrypt = require("bcryptjs");
...
const requireAdmin = async (req, res, next) => {
  try {
    ...
    res.status(403).json({ error: "Failed to verify admin" });
  }
};

const toISO = (field) =>
  field
    ? (field.toDate ? field.toDate() : new Date(field)).toISOString()
...
app.post("/register", async (req, res) => {
  try {
    ...
    const userDoc = await db.collection("users").doc(email).get();
    if (userDoc.exists) return res.status(409).json({ error: "User exists" });
    const hashed = await bcrypt.hash(password, 10);
    await db
      .collection("users")
      .doc(email)
      .set({ password: hashed, createdAt: new Date() });
...
app.post("/api/:collection", authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    const docRef = await db.collection(collection).add(payload);
    res.json({ id: docRef.id, message: "Created" });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
app.post("/upload", authenticate, upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    const folder = destinationPath.split("/")[0];
    const maxSizeMB = folderSizeLimits[folder];
    if (!maxSizeMB) return res.status(400).json({ error: "Invalid folder" });
    if (file.size > maxSizeMB * 1024 * 1024)
      return res.status(400).json({ error: `Size exceeds ${maxSizeMB}MB` });
    if (
      folder === "profiles" &&
      !destinationPath.startsWith(`profiles/${req.user.email}`)
    )
      return res.status(403).json({ error: "Profile folder only" });
    const [uploadedFile] = await bucket.upload(file.path, {
      destination: destinationPath,
    ...
    res.json({ url: publicUrl, path: destinationPath, message: "Uploaded" });
  } catch (err) {
    res.status(500).json({ error: err.message });
  } finally {
